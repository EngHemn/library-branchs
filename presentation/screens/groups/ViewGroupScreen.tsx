...
export function ViewGroupScreen({
  ...
  useDashboardBreadcrumbs([
    ...
    { label: state.group?.name ?? t("groups.view.breadcrumbFallback") },
  ])
        ...
        <div className="flex flex-1 flex-col gap-5 p-4 pt-0 md:p-6 md:pt-0">
          <section className="flex flex-col gap-4 pt-4 sm:flex-row sm:items-start sm:justify-between">
            <div className="flex items-start gap-4">
              ...
              <div className="space-y-1">
                ...
                <Badge variant={statusVariants[state.group.status]}>
                  {statusLabel(state.group.status)}
                </Badge>
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Button
                onClick={() => router.push(dashboardPaths.groups.edit(groupId))}
              >
                <PencilIcon />

                {t("groups.view.editGroup")}
              </Button>
            </div>
          </section>
          ...
          >
            <TabsList>
              <TabsTrigger value="details">
                {t("groups.view.tabs.details")}
              </TabsTrigger>

              <TabsTrigger value="books">
                {t("groups.view.tabs.books")} ({state.group.totalBooks})
              </TabsTrigger>

              <TabsTrigger value="staff">
                {t("groups.view.tabs.staff")} ({state.group.totalAssignedStaff})
              </TabsTrigger>

              <TabsTrigger value="sales">
                {t("groups.view.tabs.sales")} ({state.sales.length})
              </TabsTrigger>

              <TabsTrigger value="reports">
                {t("groups.view.tabs.reports")}
              </TabsTrigger>
            </TabsList>
