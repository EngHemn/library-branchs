...
export function useGroupsViewModel(
  ...
  const summary =
    ...
    user !== null ? buildScopedGroupSummary(scopedGroups) : null



  const groupsStatus: AsyncStatus = userQuery.isPending || groupsQuery.isPending
  ...
  async function confirmDeleteGroup(): Promise<void> {
    ...
    await deleteMutation.mutateAsync(deleteGroupDialog.groupId).catch(() => undefined)
